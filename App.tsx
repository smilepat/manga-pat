...
const App: React.FC = () => {
  ...
  const downloadPDF = (withWatermark = true) => {
    ...
    if (cover?.imageUrl) {
      ...
      doc.setFontSize(24);
      doc.text("위인전 공방", 240, 320, { align: 'center' });
    }
    ...
    storyPages.forEach((f, i) => {
      ...
      if (withWatermark) {
        ...
        doc.setFontSize(7);
        doc.text("위인전 공방 — AI 역사 전기 만화", 470, 716, { align: 'right' });
      }
    });
  ...
  return (
    <div className="comic-scene">
      ...
      <Setup
        ...
        onHeroRemove={() => { setHero(null); toast.success('위인 이미지가 해제되었습니다.'); }}
        onFriendRemove={() => { setFriend(null); toast.success('관련 인물 1 이미지가 해제되었습니다.'); }}
        onFriend2Remove={() => { setFriend2(null); toast.success('관련 인물 2 이미지가 해제되었습니다.'); }}
